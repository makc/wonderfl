// forked from keim_at_Si's MIDIシーケンスを演奏してみた
...
package
{
	...
	public class main extends Sprite
	{
		...
		function main()
		{
			...
			//demo button
			var demoButton:Sprite = b(350, 5, "Load Demo");
			demoButton.addEventListener(MouseEvent.CLICK, onLoadDemo);
		...
		private function onLoadDemo(event:MouseEvent):void
		{
			...
			loader.load(new URLRequest("http://www.dreamana.com/lab/sion/midi/" + int(Math.random() * 6 ) + ".mid"));
		}
...
class MIDIPlayer {
    ...
    private function _createGMDrumSet() : void {
        ...
        */
        ...
        //percussion (load a drumset)
        Security.loadPolicyFile("http://www.dreamana.com/crossdomain.xml");
        
        var loader:Loader = new Loader();
        loader.contentLoaderInfo.addEventListener(Event.COMPLETE, onLoadComplete);
        loader.contentLoaderInfo.addEventListener(IOErrorEvent.IO_ERROR, function():void{ trace("IOError")});
        loader.load(new URLRequest("http://www.dreamana.com/lab/sion/JV1080.swf"));  
    }
	...
	dynamic class SMFEvent
	{
		...
		public function toString():String
		{
			...
			case 0x90:
				...
				text += "o" + int(this.note / 12 - 1);
				text += ["c", "c+", "d", "d+", "e", "f", "f+", "g", "g+", "a", "a+", "b"][this.note % 12]
				text += " ";//+this.velocity;
				break;
			case 0xB0:
