...
/*----------------------------------------------------------------*/
// Initializers.
const TITLE = "MIRROR MABLE";
var score, nextEnemyScore;
...
function initializeGame() {
    ...
    mirror = new Mirror;
    marble = new Marble;
    target = new Target((0.3 + randn(0.4)) * SCREEN_WIDTH, (0.3 + randn(0.4)) * SCREEN_HEIGHT);
    enemies = new Vector.<Enemy>;
    score = 0; nextEnemyScore = 100;
    initializeStage();
}
...
function initializeStage() {
    var ma = mirror.angle, mx = mirror.pos.x, my = mirror.pos.y;
    marble.pos.x = target.pos.x; marble.pos.y = target.pos.y;
    marble.sprite.pos = marble.pos;
    for (var i = 0; i < 32; i++) {
        mirror.angle = randn(PI * 2);
        mirror.pos.x = (0.3 + randn(0.4)) * SCREEN_WIDTH;
        mirror.pos.y = (0.3 + randn(0.4)) * SCREEN_HEIGHT;
        var tp = getMirrorPos(marble.pos.x, marble.pos.y);
        if (tp.x > SCREEN_WIDTH * 0.1 && tp.x < SCREEN_WIDTH * 0.9 &&
            tp.y > SCREEN_HEIGHT * 0.1 && tp.y < SCREEN_HEIGHT * 0.9 &&
            tp.distance(marble.pos) > 100) break;
    }
    target.pos.x = tp.x; target.pos.y = tp.y;
    target.sprite.pos = target.pos;
    mirror.angle = ma; mirror.pos.x = mx;  mirror.pos.y = my;
    if (score >= nextEnemyScore) {
        enemies.push(new Enemy(marble.pos.x, marble.pos.y));
        nextEnemyScore += 100;
    }
}
// Update a game frame.
function updateGame() {
    ...
    marble.update();
    target.update();
    for each (var e in enemies) e.update();
}
function updateTitle() {
    scoreHistory.draw(SCREEN_WIDTH, 0);
    drawTitle(TITLE, new Array("WASD", "JL"), new Array("MOVE MIRROR", "TILT MIRROR"), "Z");
}
...
var mirror;
class Mirror {
    var pos = new Vec(SCREEN_CENTER, SCREEN_CENTER), angle = 0, p = new Vec;
    function update() {
        pos.incrementBy(getWasdWay(6));
        setInScreen(pos);
        var lk = getIjklWay(0.06);
        angle -= lk.x + lk.y;
        fillRect(pos.x, pos.y, 7, 7, 0xffffff);
        draw(1); draw(-1);
    }
    function draw(w) {
        p.x = pos.x; p.y = pos.y;
        while (isInScreen(p)) {
            p.addAngle(angle, 5 * w);
            fillRect(p.x, p.y, 3, 3, 0xcccccc);
        }
    }
}
// Marble.
...
class Marble extends Actor {
    const SIZE = 16;
    var mirrorMarble;
    function Marble(isMirrored = false) {
        super(PrimitiveShape.CIRCLE, SIZE);
        if (!isMirrored) {
            mirrorMarble = new Marble(true);
            sprite.alpha = 0.7;
        }
    }
    function update() {
        mirrorMarble.pos = getMirrorPos(pos.x, pos.y);
        mirrorMarble.sprite.pos = mirrorMarble.pos;
        if (mirrorMarble.pos.distance(target.pos) < SIZE * 2) target.get();
    }
}
var mirrorPos = new Vec;
function getMirrorPos(x, y) {
    var md = getMirrorDistance(x, y);
    mirrorPos.x = x; mirrorPos.y = y;
    mirrorPos.addAngle(mirror.angle + PI / 2, -md * 2);
    return mirrorPos;
}
function getMirrorDistance(x, y) {
    var ma = mirror.angle;
    var c = -(cos(ma) * mirror.pos.x - sin(ma) * mirror.pos.y);
    var md = cos(ma) * x - sin(ma) * y + c;
    return md;
}
// Target.
var target;
class Target extends Actor {
    var scoreTicks = 0, ts:int;
    function Target(x, y) {
        super(PrimitiveShape.BOX, 20);
        pos.x = x; pos.y = y;
        sprite.pos = pos; sprite.angle = PI / 4;
    }
    function update() {
        ts = 5000 / (scoreTicks + 50);
        cg.printFromCenter(String(ts), pos.x, pos.y - 30);
        scoreTicks++;
    }
    function get() {
        addParticlesRound(32, pos.x, pos.y, 20, 5, 0.8);
        addNumberBoard(ts, pos.x, pos.y - 30);
        score += ts; scoreTicks = 0;
        initializeStage();
    }
}
// Enemies.
...
class Enemy extends Actor {
    var vel = new Vec, xs, ys, br;
    function Enemy(x, y) {
        super(PrimitiveShape.CROSS, 16);
        pos.x = x; pos.y = y;
        xs = 0.001 + randn(0.009); ys = 0.001 + randn(0.009);
        br = 0.7 + randn(0.2);
    }
    function update() {
        var mp = marble.mirrorMarble.pos;
        vel.x += (mp.x - pos.x) * xs;
        vel.y += (mp.y - pos.y) * ys;
        vel.scaleBy(br);
        pos.incrementBy(vel);
        sprite.pos = pos;
        if (pos.distance(mp) < 16) {
            addParticlesRound(256, mp.x, mp.y, 32, 10);
            marble.mirrorMarble.sprite.remove();
            startGameOver();
        }
    }
}
