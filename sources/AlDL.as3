// forked from codeonwort's [Stage3D] shadow on limited plane
...
class PlaneObject {
    ...
    public var geom:PlaneGeometry
    public var vbuf:VertexBuffer3D
    public var ibuf:IndexBuffer3D
    public var colorbuf:VertexBuffer3D
    private var context:Context3D
    
    public function PlaneObject(context:Context3D, w:Number, h:Number, r:Number, g:Number, b:Number) {
        this.context = context
        geom = new PlaneGeometry(w, h)
        vbuf = context.createVertexBuffer(geom.numVertices, 3)
        vbuf.uploadFromVector(geom.vbuf, 0, geom.numVertices)
        ibuf = context.createIndexBuffer(geom.ibuf.length)
        ibuf.uploadFromVector(geom.ibuf, 0, geom.ibuf.length)
        colorbuf = context.createVertexBuffer(geom.numVertices, 3)
        colorbuf.uploadFromVector(Vector.<Number>([r,g,b, r,g,b, r,g,b, r,g,b]), 0, 4)
    }
    
    public function render(program:Program3D, order:int):void {
        context.setDepthTest(true, 'less')
        context.setStencilReferenceValue(order)
        context.setStencilActions('frontAndBack', 'always', 'set', 'keep', 'set')
        ...
        context.setProgramConstantsFromMatrix('vertex', 0, geom.transform, true)
        context.drawTriangles(ibuf, 0, geom.numTriangles)
    }
...
//{
    ...
    internal class Study_ShadowOnPlane extends Study_Base 
    {
        ...
        private var plane1:PlaneObject
        private var plane2:PlaneObject
        
        private var shadowColor:Vector.<Number> = Vector.<Number>([0, 0, 0, 0.6])
        private var lightDir:Vector3D = new Vector3D(0.5, -1, 0)
        ...
        public function Study_ShadowOnPlane(document:Sprite, context:Context3D) 
        {
            ...
            // make planes
            plane1 = new PlaneObject(context, 2, 1.7, 1,0,0)
            plane1.geom.spinX(45)
            plane1.geom.move(0, -0.2, 2)
            
            plane2 = new PlaneObject(context, 1.2, 2, 0,1,0)
            plane2.geom.spinY(60)
            plane2.geom.move(0.6, 0, 2)
        ...
        private function renderPlaneAndShadow(plane:PlaneObject, lightDir:Vector3D, order:int):void {
            plane.render(program, order)
            
            var shadowProj:Vector.<Number> = makeShadowProj(plane.geom.normal, plane.geom.innerPoint, lightDir)
            context.setDepthTest(true, 'always')
            context.setStencilReferenceValue(order)
            context.setStencilActions('frontAndBack', 'equal')
        ...
        private function render($:Object=null):void {
            ...
            renderPlaneAndShadow(plane1, lightDir, 1)
            renderPlaneAndShadow(plane2, lightDir, 2)
