// forked from ProjectNya's Cube
/**
 * Copyright ProjectNya ( http://wonderfl.net/user/ProjectNya )
 * MIT License ( http://www.opensource.org/licenses/mit-license.php )
 * Downloaded from: http://wonderfl.net/c/ze7j
 */
...
package {
	...
	public class Main extends Sprite {
		...
		private function init():void {
			...
			cube1.x = 112;
			cube1.y = 300;
			cube2 = new Cube(20, 20, 0xFF00);
			addChild(cube2);
			cube2.x = 232;
			cube2.y = 360;
			cube3 = new Cube(80, 200, 0xFF7F00);
			addChild(cube3);
			cube3.x = 352;
			cube3.y = 300;
...
class Cube extends Sprite {
	...
	public function Cube(w:uint, h:uint, color:uint = 255) {
		var c_r:uint = (color & 0xFF0000) >> 16;
		var c_g:uint = (color & 0xFF00) >> 8;
		var c_b:uint = (color & 0xFF);
		var c_y:uint = 0.299 * c_r + 0.587 * c_g + 0.114 * c_b; // could be simple average
		c_r = Math.min (255, c_r + c_y);
		c_g = Math.min (255, c_g + c_y);
		c_b = Math.min (255, c_b + c_y);

		s1Color = color;
		s2Color = (c_r << 16) + (c_g << 8) + c_b;
		cColor = s2Color;
