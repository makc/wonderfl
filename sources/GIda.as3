...
package {
	...
	public class Marimo extends Sprite {
		...
		// できそうなこと:ビットマップにピクセルで描いて縮小するようにすれば、軽くなるかも。パーティクルを制御すれば、地面と毛が接する表現になるかも
                ...
                // 見た目のために値を色々調節してはいるが、基本的に影の描画順序を奥にしたのと、drawFurに３行追加しただけ。
		...
		public function Marimo() {
			...
			gradientMatrix.createGradientBox(SHADOW_W, SHADOW_H, 0, _marimoX - SHADOW_W/2, GROUND_Y-SHADOW_H/2);
			g.beginGradientFill(GradientType.RADIAL, [0x000000, 0x000000], [0.6, 0.0], [100, 255], gradientMatrix);	// グラデーションわけわからん・・・
