...
package {
    ...
    public class main extends Sprite {
        ...
        private function renderMinecraft(e: Event): void {
            ...
            var rB:int = getTimer() % 1000>500? 255:128;
            var gB:int = getTimer() % 1000<500? 255:128;
            var bB:int = (getTimer()+250) % 1000>500? 255:128;
            ...
            for ( var x:int = 0; x < w; x++) {
                ...
                for ( var y:int = 0; y < h; y++) {
                        ...
                        while (dist < closest) {
                            ...
                            if (tex > 0) {
                                ...
                                if (cc > 0) {
                                    ...
                                    br = (d==0 ? 75 : (d==1 ? 255 : 205));
                                    if (d == 1)
                                    {
                                        var maxsh:Number = 0.7;
                                        var revShcorner:Number = 1/7;
                                        
                                        var ZPPLUS:int = ((zp+1) & 255) << 12;
                                        var ZPMINUS:int = ((zp-1) & 255) << 12;
                                        var YPMINUS:int = ((yp-1) & 63) << 6;
                                        var XPLUS:int = ((xp+1) & 63);
                                        var XMINUS:int = ((xp-1) & 63);
                                        tex = map[ ZPMINUS | YPMINUS | XMINUS ];
                                        if (tex>0)
                                            br *= Math.max(maxsh, Math.min(1.0, (u+v)*revShcorner));
                                        tex = map[ ZPPLUS | YPMINUS | XMINUS ];
                                        if (tex>0)
                                            br *= Math.max(maxsh, Math.min(1.0, (u+16-v)*revShcorner));
                                        tex = map[ ZPMINUS | YPMINUS | XPLUS ];
                                        if (tex>0)
                                            br *= Math.max(maxsh, Math.min(1.0, (16-u+v)*revShcorner));
                                        tex = map[ ZPPLUS | YPMINUS | XPLUS ];
                                        if (tex>0)
                                            br *= Math.max(maxsh, Math.min(1.0, (16-u+16-v)*revShcorner));

                                        var revSh:Number = 1/7;
                                        tex = map[((zp) & 255) << 12 | YPMINUS | XMINUS ];
                                        if (tex>0)
                                            br *= Math.max(maxsh, Math.min(1.0, (u)*revSh));
                                        tex = map[((zp) & 255) << 12 | YPMINUS | XPLUS ];
                                        if (tex>0)
                                            br *= Math.max(maxsh, Math.min(1.0, (16-u)*revSh));
                                        tex = map[ZPMINUS | YPMINUS | ((xp) & 63)];
                                        if (tex>0)
                                            br *= Math.max(maxsh, Math.min(1.0, (v)*revSh));
                                        tex = map[ZPPLUS | YPMINUS | ((xp) & 63)];
                                        if (tex>0)
                                            br *= Math.max(maxsh, Math.min(1.0, (16-v)*revSh));
                                ...
                                if (d == 1)
                               {
                                   var XZ:int = (zp & 255) << 12 | (xp & 63);
                                    tex = map[ XZ | YPMINUS ];
                                    if (tex > 0)
                                        br *= 0.7;
                                    else
                                   {
                                        tex = map[ XZ | ((yp-2) & 63) << 6 ];
                                        if (tex > 0)
                                            br *= 0.8;
                                        else
                                       {
                                        tex = map[ XZ | ((yp-3) & 63) << 6 ];
                                        if (tex > 0)
                                            br *= 0.9;
                                       }
                                   }
                    ...
                    if(col)
                        bmd.setPixel32(x, y, 0xff000000 | r << 16 | g << 8 | b);
