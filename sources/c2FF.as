/*
 * Trying to find bad coef-s values for Durand-Kerner,
 * starting at 0.1801 0.9532 0.9787 0.02861
 * 
 * (even if found, they obviously depend on seed value)
 */
package {
	...
	public class Test extends Sprite {
		...
		private var min_rs:Number = 0.02;
		...
		public function calculate (e:Event):void {
			...
			A.x = min_cs [0] + 0.1 * min_rs * (Math.random () - Math.random ());
			B.x = min_cs [1] + 0.1 * min_rs * (Math.random () - Math.random ());
			C.x = min_cs [2] + 0.1 * min_rs * (Math.random () - Math.random ());
			D.x = min_cs [3] + 0.1 * min_rs * (Math.random () - Math.random ());
			...
			out.htmlText = N + " iterations, roots:\n " + p + ",\n " + q + ",\n " + r + ",\n " + s +
				"\n\nr-s distance at iteration №2: " + min_rs +
				"\n\ncoefficients: " +
				"\n\t" + min_cs.join ("\n\t");
