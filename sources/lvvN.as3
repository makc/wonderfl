package {
	...
	public class DoublePendulum extends Sprite
	{
		...
		private var gamma1:Number = 0.0005;
		private var gamma2:Number = 0.001;
		...
		private function Advance():void
		{
			...
			theta = ModifyRadian(theta_new);
			phi = ModifyRadian(phi_new);			
		} 
		private function ModifyRadian(rad:Number):Number
		{
			while(rad >  Math.PI)
			{
				rad -= 2 * Math.PI;
