// forked from yonatan's forked from: flash on 2014-5-25
            ...
            addEventListener("enterFrame", function(e:*):void {
                ...
                for (n = cnt * 6, i = 0; i < n; i++) {
                    ...
                    for (var j:int = 0; j < cnt; j++) {
                        ...
                        var r:Number = 0.4 * Math.cos(B + t * 2 * Math.PI) + 0.6;
                        var R:Number = 1 + 0.33 * Math.sin(A) * r;
                        var H:Number = 0.2 * Math.cos(A) * r;
                        p[j].r = r + 0.5;
                    ...
                    for (var k:int = 0; k < cnt; k++) {
                        ...
                        graphics.drawCircle(p[k].X, p[k].Y, (15 * p[k].r/cnt) / (0.5 - p[k].Z));
                    }
                }
            });
        }
