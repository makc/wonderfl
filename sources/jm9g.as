// forked from keim_at_Si's wonderflで音楽♪Like the wind forked from: UpDownRoad
...
package
{
  ...
  public class UpDownRoad extends Sprite
  {
    ...
    private const DRAWN_PARTICLES_MAX_COUNT:int = 3000;
    ...
    private const ROAD_HEIGHT:Number = 7;
    private const SIGHT_HEIGHT:Number = 36;
    private const BACKGROUND_BOX_SIZE:int = 16;
    private const BACKGROUND_R:int = 0xcc;
    private const BACKGROUND_G:int = 0xdd;
    private const BACKGROUND_B:int = 0xaa;
    ...
    public function UpDownRoad()
    {
      ...
      viewpoint.x = roads[carPosIndex].pos.x;
      viewpoint.y = roads[carPosIndex].pos.y;
      viewpoint.z = roads[carPosIndex].pos.z;
    ...
    private function drawBackground():void
    {
      ...
      for (var j:int = 0; j < 3; j++)
      {
        ...
        for (var i:int = 0; i < bc; i++)
        {
          ...
          rect.y = by - bh;
          buffer.fillRect(rect, createColor(0xff - j * 0x33, 0xee, 0x88 - j * 0x33, 255));
    ...
    private var roadPattern:Array =
    [20,0,0.2,1,1, 16,0,-3,0,1, 16,0,3,0,1, 80,0.05,0.05,1,1,
     90,0,0,0,0, 80,-0.05,-1,1,1, 20,0,1.75,1,1];
    ...
    private function createRoad():void
    {
      ...
      roadStartY = r.pos.y = -70;
      r.angle = a;
      r.upDown = ud;
      var c:int;
      var ta:Number, tud:Number;
      var polePattern:int;
      var surfacePattern:int;
      var i:int;
      roadIndex = 0;
      for (i = 0; i < roadPattern.length; i += 5)
      {
        c = roadPattern[i];
        ta = roadPattern[i + 1];
        tud = roadPattern[i + 2];
        polePattern = roadPattern[i + 3];
        surfacePattern = roadPattern[i + 4];
        for (var j:int = 0; j < c; j++)
        {
          ...
          r.upDown = ud;
          addRoadAndPole(r, polePattern, surfacePattern);
          r = getNextRoad(r);
        }
      }
    ...
    private function addRoadAndPole(r:Road, polePattern:int, surfacePattern:int):void
    {
      addRoadParticles(r, surfacePattern);
      roads.push(r);
    ...
    private function addRoadParticles(r:Road, surfacePattern:int):void
    {
      var i:int, sr:int, sg:int, sb:int;
      switch (surfacePattern)
      {
        case 0:
          sr = sg = sb = 210;
          break;
      ...
      {
        addBoxParticle
        (r.pos.x + Math.cos(r.angle) * ROAD_WIDTH / 3 * 2 * (i - 1),
         r.pos.y,
         r.pos.z - Math.sin(r.angle) * ROAD_WIDTH / 3 * 2 * (i - 1),
         ROAD_WIDTH / 3 * 1.4, ROAD_HEIGHT, 4, 1, r.angle, 0,
         sr, sg, sb, 1);
      }
      if (roadIndex % 3 == 0)
      {
        for (i = 0; i < 2; i++)
        {
          ...
          p.pos.x = r.pos.x + Math.cos(r.angle) * ROAD_WIDTH * 1.2 * (i * 2 - 1);
          p.pos.y = r.pos.y - ROAD_HEIGHT * 2;
          p.pos.z = r.pos.z - Math.sin(r.angle) * ROAD_WIDTH * 1.2 * (i * 2 - 1);
          p.width = ROAD_HEIGHT;
          p.height = ROAD_HEIGHT * 0.75;
          p.r = p.g = p.b = 32;
          p.detail = null;
          particles.push(p);
        }
    ...
    private function addPoleParticles(r:Road):void
    {
      for (var i:int = 0; i < 2; i++)
        addBoxParticle(r.pos.x - Math.cos(r.angle) * ROAD_WIDTH * (i * 2 - 1) * 1.1,
                       r.pos.y - 32,
                       r.pos.z + Math.sin(r.angle) * ROAD_WIDTH * (i * 2 - 1) * 1.1,
                       6, 32, 1, 6, r.angle, 0,
                       250, 60, 60, 0);
    }
    ...
    private function addTrees():void
    {
      ...
      {
        var x:Number = (Math.random() * 2 - 1) * 1000;
        var z:Number = (Math.random() * 2 - 1) * 1000;
        if (getDistanceFromRoad(x, z) < 100)
          continue;
        var wh:Number = Math.random() * 10 + 10;
        var lh:Number = Math.random() * 20 + 20;
        addBoxParticle(x, -wh, z, 3 + Math.random() * 2, wh, 1, 3, 0, 0, 150, 100, 50, 1);
        addBoxParticle(x, -wh - lh, z, 12 + Math.random() * 5, lh, 5, 10, 0, 1, 40, 240, 20, 1);
      }
    }

    private function getDistanceFromRoad(x:Number, z:Number):Number
    {
      var d:Number = 99999;
      offset.y = 0;
    ...
    private function addBoxParticle(x:Number, y:Number, z:Number,
                                    width:Number, height:Number, wDiv:int, hDiv:int,
                                    angle:Number, anglePattern:int,
                                    r:int, g:int, b:int, colorPattern:int):void
    {
      var p:Particle = new Particle;
      ...
      var ox:Number = Math.cos(angle) * width * 2 / wDiv;
      var oz:Number = -Math.sin(angle) * width * 2 / wDiv;
      for (var dx:int = 0 ; dx < wDiv; dx++)
      {
        for (var dy:int = 0; dy < hDiv; dy++)
        {
          if (anglePattern == 1)
          {
            var a:Number = Math.random() * Math.PI * 2;
            ox = Math.cos(a) * width * 2 / wDiv;
            oz = -Math.sin(a) * width * 2 / wDiv;
          }
          var pp:Particle = new Particle;
          pp.pos.x = p.pos.x + (dx - wDiv / 2) * ox;
          pp.pos.y = p.pos.y + (dy - hDiv / 2) * height * 2 / hDiv;
          pp.pos.z = p.pos.z + (dx - wDiv / 2) * oz;
          pp.width = width / wDiv * 1.4;
          pp.height = height / hDiv * 1.1;
          switch (colorPattern)
          {
            case 0:
              ...
              break;
            case 1:
              var co:int = Math.random() * 30 - 15;
              pp.r = r + co; pp.g = g + co; pp.b = b + co;
              break;
          }
          pp.detail = null;
          p.detail.push(pp);
