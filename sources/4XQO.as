...
package  {
    ...
    public class Race extends Sprite {
        ...
        public function Race() {
            ...
            //車
            _car = createCar(false, 0xFFE31A, Dot.car1);
            ...
            for (i = 0; i < ENEMY_NUM; i++) {
                ...
                var enemy:Car = createCar(true, 0xFF0000, (index >= 9)? [Dot.car4, Dot.car3][index-9] : Dot.car2);
                enemy.accel = 8 + index / 10 * 2;
            ...
            _message = new Label(labels, 180, 200, "PRESS KEY");
        ...
        private function createCar(isCPU:Boolean = true, rgb:uint = 0xC80000, bmd:BitmapData = null):Car {
            var car:Car = new Car(_sim, isCPU, rgb);
...
class Car {
    ...
    public function Car(sim:QuickBox2D, isCPU:Boolean = true, rgb:uint = 0xC80000) {
        this.isCPU = isCPU;
        qobject = sim.addBox( { density:[1,0.3][int(isCPU)], restitution:[0,0.8][int(isCPU)], friction:0.2, fillColor:rgb, x:0, y:0, width:0.6, height:1.2 } );
    ...
    public function update(w:World):void {
        ...
        qobject.body.ApplyTorque(qobject.params.density * vec.Length() / (20 + speedRate*40) * t);
    }
}