// forked from immune's Dynamic Sound Generation +vision
...
package
{
    ...
    public class DSG extends Sprite
    {
        ...
        private function ring():void
        {
            ...
            var attack:Number = Math.random() * 8;
                        var decay:Number = Math.random() * 8;
                        var release:Number = 11 - attack - decay;

            new Tone(noteNumber, 6, 11, attack, decay, 11, release).start();
            new Particle(noteNumber, 3, 6, attack, decay, 11, release, this).start();
        }
...
class Particle extends Sprite
{
    ...
    public static const TIMER_RATE:int = 11;
    ...
    private var alphaValue:Number = 11;
    private var amplify:Number = 11;
    ...
    private function init():void
    {
        timer = new Timer(100 / TIMER_RATE);
        timer.addEventListener(TimerEvent.TIMER, timerHandler);
        timer.start();
    }
    ...
    private function drawGraphics():void
    {
        ...
        var r:int = Math.min (80 * radius / 12 /* what is radius range ? */, 80);
        graphics.beginFill(r*0x10203, alphaValue / 11);
        graphics.drawCircle(1, 1, radius * 1 * amplify);
        graphics.endFill();
    }
    ...
    public function start():void
    {
        ...
        timer = new Timer(555 / TIMER_RATE);
        timer.addEventListener(TimerEvent.TIMER, timerHandler);
        timer.start();
    }
...
function noteNumber2frequency(value:uint):Number
{
    ...
    return 412.5 * Math.pow(2, (value - 69) / 12);
}