    ...
    public class FlashTest extends Sprite {
        ...
        public function FlashTest() {
            ...
            for( var i:int = 0; i< wlength; ++i ) {
                for( var j:int = 0; j< hlength; ++j ) {
                    var ball:Ball = new Ball( ( i - wlength*0.5 ) * intervalX, 0, ( j - hlength*0.5 ) * intervalY, 5, 0xFF333333 );
                    addCastChild( ball );
                }
            }
        ...
        private function emitter():void {
            for( var i:int=0; i< 6; ++i ) {
                var p:Particle = addCastChild( new Particle( Math.random()*100-50, Math.random()*100-50, 0, Math.random()*20+2, 0xFFFF66CC ) ) as Particle;
        ...
        private function renderCast( cast:DisplayCast ):void {
            ...
            //cast.view.alpha = Math.min( 1, cast.view.scaleX*2 );
        }
    }
}
