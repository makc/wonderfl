...
package {
    ...
    public class Main extends Sprite {
        ...
        private var initializeButton:TextField;
        private var turn_tf:TextField;
        ...
        public function Main() {
            ...
            initializeButton = new TextField();
            initializeButton.x = 0;
            initializeButton.y = tf.y;
            //initializeButton.width = stage.stageWidth / 2;
            initializeButton.text = "init";
            initializeButton.addEventListener(MouseEvent.CLICK, init);
            addChild(initializeButton);
            
            turn_tf = new TextField();
            addChild(turn_tf);
            
            dialog = new Dialog(stage.stageWidth, stage.stageHeight);
            dialog.visible = false;
            dialog.addEventListener(MouseEvent.CLICK, function(){dialog.visible = false;});
            addChild(dialog);
        ...
        private function init(event:Event = null):void {
            ...
            model.init();
            view.init();
            tf.text = "Player1: 0  Player2: 0";
            turn_tf.text = "Player1's turn";
            myTurn();
        }
        ...
        private function myTurn():void {
            ...
            turn_tf.text = "Player1's turn";
            reciever.removeEventListener("submit_hand", playComHand);
        ...
        private function comTurn():void {
            reciever.willSuggest = true;
            reciever.clickable = true;
            turn_tf.text = "Player2's turn";
            reciever.removeEventListener("submit_hand", playMyHand);
            reciever.addEventListener("submit_hand", playComHand);
        }
        
        private function playComHand(event:JintoriEvent = null):void {
            if (model.exists(event.x, event.y, event.z)) return;       
            var ps:Array = model.put(event.x, event.y, event.z, JintoriModel.BLACK);
            view.addLine(event.x, event.y, event.z);
        ...
        private function onGameEnd():void {
            ...
            if (countW > countB) dialog.setMessage("Player1 Win");
            else if (countW < countB) dialog.setMessage("Player2 Win");
...
class Dialog extends Sprite {
    ...
    public function Dialog(w:int, h:int) {
        ...
        _tf2.text = "Click to close this dialog";
