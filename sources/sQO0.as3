...
package {
	...
	public class Jobs extends Sprite {
		...
		private function init(e:Event = null):void {
			...
			bd = new BitmapData (info.content.width, info.content.height);
			bd.perlinNoise (100, 100, 3, 123, true, false);

			occlusion.addChild (info.content);

			fx = new EffectContainer (info.content.width, info.content.height, emission, occlusion);
			fx.setViewportSize (info.content.width, info.content.height);
			fx.srcX = 0.6 * info.content.width;
			fx.srcY = 0.7 * info.content.height;
			addChild (fx);

			x = 0.5 * (465 - info.content.width);
			y = 0.5 * (465 - info.content.height);
		...
		private function frame(e:*):void {
			mat.translate (-1.5, 2);
			emission.graphics.clear ();
			emission.graphics.beginBitmapFill (bd, mat);
			emission.graphics.drawRect (0, 0, bd.width, bd.height);
