...
// Initializers.
var score:int, ticks:int, stage:int, stageScore:int;
...
function updateGame():void {
    ...
    player.update();
    if (enemies.length <= 0) {
        for (var i:int = 0; i < 1 + sawtoothWave(Number(stage) / 5, 0.2) * 5; i++) enemies.push(new Enemy());
        stage++; stageScore = 100;
    }
    for (i = 0; i < bullets.length; i++) if (!bullets[i].update()) {
        bullets[i].sprite.remove();
        bullets.splice(i--, 1);
    }
    for (i = 0; i < enemies.length; i++) if (!enemies[i].update()) enemies.splice(i--, 1);
    stageScore--; if (stageScore < 1) stageScore = 1;
    ticks++;
}
function updateTitle():void {
    scoreHistory.draw(SCREEN_WIDTH, 0);
    drawTitle("PISTOL360", new Array("WASD", "IJKL"), new Array("MOVE", "FIRE"), "Z");
}
...
var player:Player;
class Player {
    public static const SPEED:Number = 8, SIZE:Number = 16;
    ...
    public var rect:Rectangle = new Rectangle(0, 0, 3, 3);
    public var rp:Vector3D = new Vector3D;
    public var rra:Number = PI;
    public function Player() {
        if (!shape) shape = new PrimitiveShape(PrimitiveShape.HUMAN, SIZE, SIZE);
        sprite = new PrimitiveSprite(shape);
        pos.x = pos.y = SCREEN_WIDTH / 2;
    }
    public function update():void {
        var l:Vector3D = getWasdWay(SPEED);
        pos.incrementBy(l);
        setInScreen(pos);
        sprite.pos = pos;
        var r:Vector3D = getIjklWay();
        if (r.length > 0) {
            rra = aim(rra, atan2(r.x, r.y), 0.1);
            rp.x = pos.x; rp.y = pos.y;
            while (isInScreen(rp)) {
                addAngleVector(rp, rra, Bullet.SIZE);
                rect.x = rp.x - 1; rect.y = rp.y - 1;
                bd.fillRect(rect, 0xaaaaaa);
                for each (var e:Enemy in enemies) {
                    if (e.isDestroyed || e.isTargeted) continue;
                    if (distance(e.pos, rp) < Bullet.SIZE * 2) bullets.push(new Bullet(pos, rra, e));
                }
            }
        }
        for each (e in enemies) {
            if (!e.isDestroyed && distance(e.pos, pos) < SIZE) startGameOver();
        }
    }
}
// Enemies.
...
class Enemy {
    ...
    public var isDestroyed:Boolean, isTargeted:Boolean;
    public function Enemy() {
        if (!shape) shape = new PrimitiveShape(PrimitiveShape.CIRCLE, SIZE, SIZE);
        sprite = new PrimitiveSprite(shape);
        if (randi(2) == 0) {
            pos.x = randn(SCREEN_WIDTH);
            if (randi(2) == 0) pos.y = 0;
            else pos.y = SCREEN_HEIGHT;
        } else {
            pos.y = randn(SCREEN_HEIGHT);
            if (randi(2) == 0) pos.x = 0;
            else pos.x = SCREEN_WIDTH;
        }
        var tx:Number = SCREEN_WIDTH * (0.3 + randn(0.4));
        var ty:Number = SCREEN_HEIGHT * (0.3 + randn(0.4));
        addAngleVector(vel, atan2(tx - pos.x, ty - pos.y), 2 + randn(1 + sawtoothWave(Number(stage) / 10) * 10));
    }
    ...
    public function destroy():void {
        ...
        addParticlesRound(20, pos.x, pos.y, 10, 7);
        score += stageScore;
        addNumberBoard(stageScore, pos.x, pos.y - 10, 0, -40);
        sprite.remove();
        isDestroyed = true;
    }
}
...
class Bullet {
    public static const SIZE:Number = 10, SPEED:Number = 32;
    public static var shape:PrimitiveShape;
    public var sprite:PrimitiveSprite;
    public var pos:Vector3D = new Vector3D;
    public var vel:Vector3D = new Vector3D;
    public var targetEnemy:Enemy, prevDist:Number;
    public function Bullet(p:Vector3D, angle:Number, targetEnemy:Enemy) {
        if (!shape) shape = new PrimitiveShape(PrimitiveShape.BAR, SIZE, SIZE);
        sprite = new PrimitiveSprite(shape);
        pos.x = p.x; pos.y = p.y;
        sprite.angle = angle;
        addAngleVector(vel, angle, SPEED);
        addParticles(10, pos.x, pos.y, vel.x, vel.y, 4, 0.9);
        this.targetEnemy = targetEnemy;
        targetEnemy.isTargeted = true;
        prevDist = distance(targetEnemy.pos, pos);
    }
    public function update():Boolean {
        sprite.pos = pos;
        pos.incrementBy(vel);
        addParticles(3, pos.x, pos.y, -vel.x / 2, -vel.y / 2, 3, 0.9, 0.1);
        var d:Number = distance(targetEnemy.pos, pos);
        if (d > prevDist || !isInScreen(pos)) {
            targetEnemy.destroy();
            return false;
        }
        prevDist = d;
