...
/*----------------------------------------------------------------*/
const TITLE = "EXPAND BOXES";
const KEYS = [">", "^"];
const OPERATIONS = ["EXPAND HORIZONTALLY", "EXPAND VERTICALLY"];
var rank, wScale;
function initialize() {
...
function update() {
    rank = 1 + sawtoothWave(Number(score) / 1000, 0.4) * 2;
    if ((isDPressed || isSPressed) && wScale < 1.8) wScale += 0.05;
    if ((isWPressed || isAPressed) && wScale > 0.2) wScale -= 0.05;
    human.update();
    updateActors(boxes);
    scroll(rank * 2);
    score++;
...
class Human extends Actor {
    ...
    var vy = 0, isEnd = false;
    function Human() {
        super(PrimitiveShape.HUMAN, SIZE);
        pos.x = SCREEN_WIDTH / 3; pos.y = 0;
    }
    function update() {
        if (isEnd) return;
        if (pos.x < SCREEN_WIDTH / 3) pos.x += 5;
        pos.y += vy * 2; vy += 0.2; vy *= 0.95;
        if (pos.y < 0) { pos.y = 0; if (vy < 0) vy *= -0.5; }
        sprite.pos = pos;
        addParticle(pos.x, pos.y, -rank * 2, 0, 2, 0.98);
        if (pos.x < -SIZE / 2 || pos.y > SCREEN_HEIGHT + SIZE / 2) {
            sprite.remove();
            startGameOver();
            isEnd = true;
        }
...
class Box extends Actor {
    ...
    function Box(x) {
        baseWidth = rands(0.1, 0.2); baseHeight = rands(0.1, 0.2);
        pos.x = SCREEN_WIDTH - x + baseWidth; pos.y = rands(0.3, 0.6);
    }
    function update() {
        var width = baseWidth * wScale;
        var height = baseHeight * (2 - wScale);
        fillRect(pos.x, pos.y, width + 2, height + 2, 0x777777);
        fillRect(pos.x, pos.y, width, height, 0xcccccc);
        if (human.pos.x + Human.SIZE / 2 > pos.x - width / 2 &&
            human.pos.x - Human.SIZE / 2 < pos.x + width / 2 &&
            human.pos.y + Human.SIZE / 2 > pos.y - height / 2 &&
            human.pos.y - Human.SIZE / 2 < pos.y + height / 2) {
            var nx1, ny1, nx2, ny2;
            if (human.pos.y < pos.y) {
                ny1 = pos.y - height / 2 - Human.SIZE / 2;
            } else {
                ny1 = pos.y + height / 2 + Human.SIZE / 2;
            }
            ny2 = human.pos.y;
            nx1 = human.pos.x;
            if (human.pos.x < pos.x) {
                nx2 = pos.x - width / 2 - Human.SIZE / 2;
            } else {
                nx2 = pos.x + width / 2 + Human.SIZE / 2;
            }
            if (abs(nx1 - human.pos.x) + abs(ny1 - human.pos.y) < abs(nx2 - human.pos.x) + abs(ny2 - human.pos.y)) {
                if (human.pos.y < pos.y) {
                    if (human.vy > 0) human.vy *= -0.5;
                    if (isWPressed || isAPressed) {
                        human.vy -= 7;
                    }
                    addParticles(10, human.pos.x, human.pos.y, -rank * 2, human.vy);
                } else {
                    if (human.vy < 0) human.vy *= -0.5;
                    addParticlesRound(200, pos.x, pos.y, 10, 10, 0.9);
                    return false;
                }
                human.pos.x = nx1; human.pos.y = ny1;
            } else {
                human.pos.x = nx2; human.pos.y = ny2;
            }
        }
        return (pos.x > -baseWidth);
    }
}
var nextBoxDistance;
function scroll(x) {
    for each (var b in boxes) b.pos.x -= x;
    nextBoxDistance -= x;
    while (nextBoxDistance <= 0) {
        boxes.push(new Box(-nextBoxDistance));
        nextBoxDistance += 0.1 * SCREEN_HEIGHT;
    }
}
/*----------------------------------------------------------------*/
