...
package {
    ...
    public class FlashTest extends Sprite {
        ...
        public function FlashTest() {
            // write as3 code here..
            //emitter = new Emitter();
            emitter = new Emitter(0,0,0,-10,0,0.01,0.9,0.9,10,200,1.0);
...
class Emitter extends EventDispatcher {
    ...
    public var explosion:Number = 2.0;
    public var vibration:Number = 0.0;
    
    public function Emitter(x:Number = 0, y:Number = 0, pvx:Number = 0, pvy:Number = 0, pax:Number = 0, pay:Number = 0, prx:Number = 1.0, pry:Number = 1.0, amounts:uint = 1, lifeTime:uint = 1, motionInfluence:Number = 0.0)
    {
        this.x = this.px = x;
        this.y = this.py = y;
        this.pvx = pvx;
    ...
    public function emit():void
    {
        ...
        for(iEmit = 0; iEmit < amounts; iEmit++)
        {
            ...
            vibx = vibration * r * Math.random();
            r = (Math.round(Math.random())) ? -1 : 1;
            viby = vibration * r * Math.random();
            particles.push(new Particle(x, y, pvx + ivx + evx + vibx , pvy + ivy + evy + viby, pax, pay, prx, pry, lifeTime));
        }
    }
