package {
	...
	public class NormalizerTest extends Sprite {
		...
		public function loop (e:Event):void {
			...
			// + max values
			for (i = 0; i < 100; i++) {
				graphics.drawRect (30 + i * 4, 120 -100 * Normalizer (b [i]).maximum (), 4, 1);
			}
			graphics.endFill ();
...
class Normalizer {
	...
	private var max:Number = e;
	private var val:Number = 0;

	public function maximum ():Number { return max; }
	public function lastValue ():Number { return val; }

	public function f (x:Number):Number {
		if (x < max) {
			max = d * max + (1 - d) * x; if (max < e) max = e;
			val = x / max;
		} else {
			max = x; if (max < e) max = e;
			val = d * val + (1 - d) * x / max;
		}
		return val;
	}
