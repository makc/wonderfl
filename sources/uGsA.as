...
package 
{
    ...
    public class Main extends Sprite 
    {
        ...
        private function init(e:Event = null):void 
        {
            ...
            _creature.Generation(stage.stageWidth / 2, stage.stageHeight / 2);
        ...
        private function Update(e:Event) : void
        {
            ...
            var move:Boolean = (_targetAge == 0);
            move = _creature.Update( _target.x, _target.y ) || move;
            if (move) {
                moveTarget();
            }
        ...
        private function moveTarget() : void { 
          _target.x = stage.stageWidth * Math.random(); 
          _target.y = stage.stageHeight * Math.random(); 
          _targetAge = 1;
        }                                                                                                             
        private function drawTarget() : void { 
          _sprite.graphics.beginFill(0xFFFFFF); 
          for (var i : int = 0;i < 3;i++) { 
            _sprite.graphics.drawCircle(_target.x, _target.y, 4 * (1.5 + Math.sin(_targetAge * 0.03 * i))); 
          } 
        }
    }
...
class Creature
{
    ...
    public function Update(tx:Number,ty:Number,range:Number=20) : Boolean
    {
        var i:int = 0;
        for ( i = 0; i < PART_NUM; i++ )
        {
        ...
        // return true if within range from the target 
        var head : CreaturePart = _partlist[0]; 
        return ((head.x - tx) * (head.x - tx) + (head.y - ty) * (head.y - ty) < range * range);
    }
