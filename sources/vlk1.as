// forked from yonatan's IT'S ALIVE! (click to cycle)
...
package  
{
	...
	public class Main extends Sprite
	{
		...
		private static const NUM_MATRICES:uint = 4;	
		...
		public function draw(): void
		{
			...
			function blend(a:uint, b:uint):uint {
                        return(
			...
			for( var i:uint = 0; i < NUM_PARTICLES; i++ ) {
				mi = Math.random() * NUM_MATRICES;
				point = matrices[mi].transformPoint(point);
        		        point.x += mw;
				point.y += mh;
				color = blend(color,colors[mi]);
				if( point.x >= 0 
					&& point.y >= 0 
					&& point.x < w 
					&& point.y < h )
		...
		public function change(): void
		{
			...
			matrices = new Array;
			colors = new Array;
			for( var i:uint = 0; i < NUM_MATRICES; i++ ) {
				matrices.push( rndMatrix() );
				colors.push( Math.random()*0xFFFFFF | 0x202020 );
			}
		}
	}
}
