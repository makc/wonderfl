...
package {
    ...
    public class FlashTest extends Sprite {
        ...
        public function FlashTest() {
            // write as3 code here..
            //emitter = new Emitter();
            emitter = new Emitter(0,0,0,0,0,0.5,0,0,3,3,100,100,0,1);
...
class Emitter extends EventDispatcher {
    ...
    private var trailNozzle:Vector.<Point>;
    ...
    public function emit():void
    {
        ...
        ta = (Math.sqrt(vx * vx + vy * vy) > velocityMin) ? amounts : 0; 
        
        //Make Trail Nozzle Vector
        trailNozzle = new Vector.<Point>();
        ...
        if(lx >= ly)
        {
            ...
            for(ti; ti <= lx; ti++)
            {
                trailNozzle.push(new Point(sx * ti, sy * tj));
                e += tty;
                if(e >= ttx)
                {
                    tj++;
        ...
        else
        {
            ...
            for(ti; ti <= ly; ti++)
            {
                trailNozzle.push(new Point(sx * tj, sy * ti));
                e += ttx;
                if(e >= tty)
                {
                    tj++;
        ...
        for(iEmit = 0; iEmit < ta; iEmit++)
        {
            ...
            var tP:Point = trailNozzle[uint(trailNozzle.length * Math.random())];
            
            particles.push(new Particle(tP.x + px, tP.y + py, pvx + ivx + evx + vibx , pvy + ivy + evy + viby, pax, pay, prx, pry, lifeTime - int(Math.random() * lifeTimeRandom)));
