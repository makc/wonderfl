...
class Star {
    ...
    public function Star() {
        ...
        blinkspan = int(Math.random() * 50) + 5;
    }
}

class NightSky extends Entity {
    private var starlist:/*Star*/Array;
    ...
    override public function update() : void {
        ...
        var g:Graphics = starshape.graphics;
        
        g.clear();
        var starnum:int = starlist.length;
        for ( var i:int = 0; i < starnum; i++ )
        {
            if ( step % starlist[i].blinkspan != 0 ){    g.beginFill(0xFFFFFF,1.0);    }
            else {                                         g.beginFill(0xFFFFFF,0.3);    }
            
            g.drawCircle( starlist[i].x, starlist[i].y, starlist[i].size );
...
class Color {
    /**雲の色*/
    static public var cloudBase:uint = 0x726070;
    /**雲のハイライト色*/
