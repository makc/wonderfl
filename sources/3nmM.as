...
package {
    ...
    public class main extends Sprite {
        ...
        function main() {
            ...
            _loader.load(
                new URLRequest("http://farm1.static.flickr.com/140/363510237_7848ccb4ff.jpg"), 
        ...
        private function init(e:Event) : void {
            ...
            _envMap = new BitmapData(256, 256, false);
            _envMap.draw(bmp.bitmapData, bmp.transform.matrix);

            bmp.width = bmp.height = 465;
            addChild(bmp);

            _materials = Vector.<Material>([
                    (new Material()).setColor(0x8080c0, 1,128, 128,   0,  0, 0,   false, _envMap),   // one color
                    (new Material()).setColor(0x8080c0, 1, 64, 128, 128, 12, 0,   false, _envMap),   // standard
                    (new Material()).setColor(0x8080c0, 1, 64, 192,   0,  0, 0,   false, _envMap),   // matte
                    (new Material()).setColor(0x8080c0, 1, 16,  64, 192, 18, 0,   false, _envMap),   // speculer
                    (new Material()).setColor(0x8080c0, 1, 16,  32, 256, 24, 0,   false, _envMap),   // more speculer
                    (new Material()).setColor(0x8080c0, 1,  0, 255,   0,  0, 0,   false, _envMap),   // diffusion only
                    (new Material()).setColor(0x8080c0, 1,  0,   0, 255, 12, 0,   false, _envMap),   // speculer only
                    (new Material()).setColor(0x8080c0, 1,  0,   0,   0,  0, 255, false, _envMap), // one color (same)
                ]);
        ...
        private function _onEnterFrame(e:Event) : void {
            ...
            if (++_timeCount == 30) {
                _textField.text = "Click to change material. Redering time: " + String(_timeSum) + "[ms/30frames]";
                _timeSum = 0;
                _timeCount = 0;
            }
        }
...
class Material {
    ...
    public function setColor(color:uint, alpha_:Number= 1.0, 
        amb:int=64, dif:int=192, spc:int=0, pow:Number=8, emi:int=0, doubleSided:Boolean=false, env:BitmapData=null) : Material
    {
