...
package {
    ...
    public class MetaBalls3D extends Sprite {
        ...
        private var skyBmd:BitmapData = new BitmapData(465, 232, false, 0);
        private var sea:Sea = new Sea(skyBmd);
        ...
        public function MetaBalls3D () {
            ...
            graphics.beginGradientFill("linear",[0x202020,0x181818,0x101010,0x080808],[1,1,1,1],[0,64,128,255],new Matrix(0.0001,0.0977,-0.1416,0.0001,80.0000,116.0000),"pad","rgb",0);
        ...
        private function draw (e:Event):void {
            ...
            // update sea
            skyBmd.fillRect(skyBmd.rect, 0);
            skyBmd.draw(canvas, new Matrix(1,0,0,1,232,150));//32+30));
            //sea.amp = 10 + 40 * (player.channel.leftPeak + player.channel.rightPeak);
            sea.update();
        }
...
class Sea extends Sprite {
    private const nearScale:Number = 0.1;
    ...
    private var output:BitmapData = new BitmapData(465, (465+1)/2, false);
    private var noise:BitmapData = new BitmapData(465/2, (465+3)/4, true);
    private var upsideDown:BitmapData = new BitmapData(465, (465+1)/2, true);
    private var gradient:BitmapData = new BitmapData(465, (465+1)/2, false);
    private var seaShape:Shape = new Shape;
    private var seaBmd:BitmapData = new BitmapData(465, (465+1)/2, true);
    ...
    private const upsideDownMtx:Matrix = new Matrix(1, 0, 0, -1, 0, 465/2);
    private var dispFilter:DisplacementMapFilter = new DisplacementMapFilter(seaBmd, null, 0, 8, 0, 150, "clamp");
    ...
    public function Sea(backdrop:BitmapData) {
        vertices.push(0, 0, 465-1, 0, 0, 465/2-1, 465-1, 465/2-1);
        uvt.push(
            0, 0, nearScale, 
            1, 0, nearScale, 
            (1-nearScale)/2, 1, 1,
        ...
        s.graphics.beginGradientFill("linear",[0x808080,0xc0c0c0,0xe0e0e0,0xffffff],[1,1,1,1],[0,64,128,255],new Matrix(0.0001,0.0977,-0.1416,0.0001,80.0000,116.0000),"pad","rgb",0);
    ...
    private function initNoise():void {
        for(var i:int = 0; i < octaves; i++) {
            var bmd:BitmapData = new BitmapData(465/2, (465+3)/4, true, 0);
    ...
    public function update():void {
        ...
        upsideDown.draw(backdrop, new Matrix(1, 0, 0, -1, 0, 465/2), new ColorTransform(0.25,0.25,0.25));
        upsideDown.draw(seaBmd, null, null, "add");
        output.applyFilter(upsideDown, upsideDown.rect, upsideDown.rect.topLeft, dispFilter);
        output.draw(gradient, null, null, "multiply");
    }
}
