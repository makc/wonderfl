...
function EveryFrame(event:Event):void{
    ...
    box.moveForward(100);
    var carRotY:Number = Math.atan2(box.y-carposition[1],
    Math.sqrt(
    Math.pow(box.x-carposition[0],2)+Math.pow(box.z-carposition[2],2)
    )
    );
    ...
    box.x += Math.sin(carRotZ)*carHorizontal*Math.cos(carposition[3])*2;
    box.y += Math.sin(carposition[3])*carHorizontal*2;
    box.z += Math.sin(carRotZ-Math.PI/2)*carHorizontal*Math.cos(carposition[3])*2;
    ...
    camera.x += ((box.x-Math.sin(carRot)*200*Math.cos(carRotY))-camera.x)/4;
    camera.y += ((box.y+Math.sin(carRotY+Math.PI/8)*200)-camera.y)/4;
    camera.z += ((box.z-Math.cos(carRot)*200*Math.cos(carRotY))-camera.z)/4;
    ...
    var forwardposition2:Array = TrackMath(courseposition+0.4);
    ...
    box.z += Math.sin(carRotZ-Math.PI/2)*carHorizontal*Math.cos(carposition[3])*2;
    */
    var RotZ:Number = Math.atan2(forwardposition[2]-currentposition[2],forwardposition[0]-currentposition[0]);
    var RotZ2:Number = Math.atan2(forwardposition2[2]-forwardposition[2],forwardposition2[0]-forwardposition[0]);
    
    
    currentposition[0] += Math.sin(RotZ)*carHorizontal*Math.cos(currentposition[3])*2;
    currentposition[1] += Math.sin(currentposition[3])*carHorizontal*2;
    currentposition[2] += Math.sin(RotZ-Math.PI/2)*carHorizontal*Math.cos(currentposition[3])*2;
  
    forwardposition[0] += Math.sin(RotZ2)*carHorizontal*Math.cos(forwardposition[3])*2;
    forwardposition[1] += Math.sin(forwardposition[3])*carHorizontal*2;
    forwardposition[2] += Math.sin(RotZ2-Math.PI/2)*carHorizontal*Math.cos(forwardposition[3])*2;
    ...
    carHorizontal = Math.min(carHorizontal,currentposition[4]/2-10);
    carHorizontal = Math.max(carHorizontal,-currentposition[4]/2+10);
    ...
    for each(p in boxArray){
        ...
        if(h>widthparticles){
            if(CourseDirection > 0){
                ...
                positions = TrackMath(i/(particles/lookForward)+Math.floor(courseposition*thing)/thing-0.5);
            }else{
                 // Camera looking back
                positions = TrackMath(i/(particles/lookForward)+Math.floor(courseposition*thing)/thing-0.5-(lookForward/2));
             }
        ...
        if(h == -widthparticles || h == widthparticles){
            // later make this use the normal
            p.py += 12;
        }
    }
