...
package {
    ...
    public class Main extends Sprite {
        ...
        public function Main():void {
            ...
            armL=new Cube3D(20,50,20);
            armL.vertsOffset(0,-50,0);
            jointArmL.addChild(armL);
            var jointArmR:Joint3D = new Joint3D();
            jointArmR.x = -60;
            jointArmR.y = -40;
            body.addChild(jointArmR);
            armR=new Cube3D(20,50,20);
            armR.vertsOffset(0,50,0);
...
class Scene3D extends Sprite {
    ...
    public function render():void {
            ...
            if (object.verts.length) {
                ...
                for (var i:uint = 0; i < numParts; i++) {
                    ...
                    for (var j:uint = 0; j < 3; j++) {
                        nX += object.realVerts3D[part.indices[j]*3+0];
                        nY += object.realVerts3D[part.indices[j]*3+1];
                        nZ += object.realVerts3D[part.indices[j]*3+2];
        ...
        dispParts.sortOn("distance", Array.DESCENDING | Array.NUMERIC);
...
class Camera3D extends Sprite {
    ...
    public function Camera3D():void {
        ...
        proj.focalLength=distance;
        matrix3D.appendTranslation(0,0,distance);
