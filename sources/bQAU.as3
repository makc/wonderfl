/**
 * Sphere made from planes, another combination.
...
package {   
    ...
    public class Main extends Sprite {   
        ...
        private function createGeometry():void {   
            ...
            var alphaStep:Number = Math.PI/(numParallels + 1)/2;   
            for (var pIndex:int = 0; pIndex < 2*numParallels + 1; pIndex += 2) {   
                var alpha:Number = alphaStep*(pIndex + 1);   
				var numMeridians:int = 10*Math.sin(alpha)/Math.sin(alphaStep)/2;   
