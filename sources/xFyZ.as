...
// Initializers.
var score:int, ticks:int, left:int, leftRect:Rectangle, leftPoint:Point;
...
function initializeGame():void {
    ...
    player = new Player;
    if (!leftRect) {
        leftRect = new Rectangle(0, 0, Player.shape.width + BLUR_SIZE * 2, Player.shape.height + BLUR_SIZE * 2);
        leftPoint = new Point(0, -BLUR_SIZE + 2);
    }
    hands = new Vector.<Hand>(2);
    var h:Hand = new Hand;
    h.length = SCREEN_WIDTH * 0.3; 
    h.angle = PI * 3 / 2; h.angleVel = -PI / 60;
    hands[0] = h;
    h = new Hand;
    h.length = SCREEN_WIDTH * 0.2;
    h.angle = PI / 6 * 4 - PI / 6 / 60 * 45; h.angleVel = hands[0].angleVel / 12;
    hands[1] = h;
    bonus = new Bonus;
    score = ticks = 0; left = 2;
}
...
function updateGame():void {
    ...
    player.update();
    var a:Number = PI - PI / 6;
    for (var i:int = 1; i <= 12; i++, a -= PI / 6) {
        cg.printFromCenter(String(i), (0.5 + sin(a) * 0.35) * SCREEN_WIDTH, (0.5 + cos(a) * 0.35) * SCREEN_WIDTH);
    }
    for each (var h:Hand in hands) h.update();
    leftPoint.x = -BLUR_SIZE + 2;
    for (i = 0; i < left; i++, leftPoint.x += 20) {
        bd.copyPixels(Player.shape.bd, leftRect, leftPoint);
    }
    ticks++;
}
function updateTitle():void {
    scoreHistory.draw(SCREEN_WIDTH, 0);
    drawTitle("CLOCK JUMP", new Array("<>", "^", "v"), new Array("MOVE", "JUMP", "FALL FAST"), "Z");
}
...
class Player {
    ...
    public function Player() {
        ...
        pos.x = SCREEN_WIDTH / 2; pos.y = 0;
    }
    public function update():void {
        var l:Vector3D = getWasdWay();
        vel.x += l.x;
        isUpPressed = l.y < 0;
        vel.y += 1 + l.y * 0.75;
        pos.incrementBy(vel); vel.scaleBy(0.9);
        if (pos.y < 0 && vel.y < 0) vel.y *= -0.5;
        if ((pos.x < 0 && vel.x < 0) || (pos.x > SCREEN_WIDTH && vel.x > 0)) vel.x *= -0.5;
        sprite.pos = pos;
        addParticle(pos.x, pos.y, -vel.x / 3, -vel.y / 3, 3);
        if (distance(pos, bonus.pos) < SIZE * 2) {
            addNumberBoard(100, pos.x, pos.y - 10, 0, -40);
            score += 100;
            bonus.setPosition();
        }
        score++;
        if (pos.y > SCREEN_HEIGHT) {
            addParticles(25, pos.x, pos.y, 0, -20, 7, 0.98, 0.8);
            if (--left < 0) startGameOver();
            else {
                pos.x = SCREEN_WIDTH / 2; pos.y = 0;
                vel.x = vel.y = 0;
            }
...
// Clock's handes.
var hands:Vector.<Hand>;
class Hand {
    public static const DOT_SPACING:Number = 10;
    public var angle:Number, angleVel:Number, length:Number;
    public var rect:Rectangle = new Rectangle(0, 0, 5, 5);
    public var pos:Vector3D = new Vector3D;
    public function update():void {
        angle += angleVel * (1 + ticks * 0.001);
        pos.x = SCREEN_WIDTH / 2; pos.y = SCREEN_HEIGHT / 2;
        for (var i:int = 0; i < length / DOT_SPACING; i++) {
            rect.x = pos.x - 2; rect.y = pos.y - 2;
            bd.fillRect(rect, 0xffffff);
            if (distance(pos, player.pos) < Player.SIZE * 1.5) {
                var a:Number = getAngle(pos, player.pos);
                var v:Number = -3; if (player.isUpPressed) v *= 4;
                addAngleVector(player.vel, a, v);
            }
            addAngleVector(pos, angle, DOT_SPACING);
        }
    }
}
// Bonus item.
...
class Bonus {
    ...
    public function setPosition():void {
        do {
            pos.x = (0.25 + randn(0.5)) * SCREEN_WIDTH;
            pos.y = (0.05 + randn(0.3)) * SCREEN_HEIGHT;
        } while (distance(player.pos, pos) < SCREEN_WIDTH * 0.2);
        sprite.pos = pos;
    }
}
/*----------------------------------------------------------------*/
...
function getAngle(p1:Vector3D, p2:Vector3D):Number {
