  ...
  * Shoot a missile by typing and 10 hit to win.
  * 
  * You can type enemy's missile word in the defence mode.
  * Protect your own base!
  * 
  * Type the roma strings when you attack with Japanese words.
  * 
  * You can attack any words appears in the game.
  * However, the missile's damage will be halfed unless
  * you type the word directly above the input field.
  * And typing the word directly above the input field gives you attack bonus.
  * 
  * Accurate typing gives you combo bonus.
  * 
  * Attack mode:
  * Perfect typing: Attack combo + 1  
  * Got typo but fixed: No change in combo number
  * Got typo and attempted to shoot: Reset combo
  * 
  * Defence mode: 
  * Perfect typing: Defence combo + 1
  * Got typo: Reset combo
  * 
  * You gain additional bonus rate when combo number reached 15.
  * Attack bonus: you can shoot more than one missiles each shooting.
  * Defence bonus: you automatically reflect the word you typed.
  * 
  * * Hot keys in battle
  * F2　start chatting
  * TAB toggle attack/defence mode
  * ESC reset inputting defence word
  * LEFT/RIGHT move your battery left or right
  * UP change to attack mode
  * DOWN change to defence mode
  * SPACE alternative ENTER or TAB ( you can set this in SETTING )
  * F12 toggle defence range
  * 
  * * SETTING
  * YOUR NAME
  * You can change you screen name.
  * 
  * ATTACK LANGUAGE
  * You can choose attack languages from Japanese, English or both of them.
  * 
  * COLOR SETTING
  * You can change your battery's color.
  * The number of colors is 20 in maximum.
  * 
  * If you see your name as Name(5/6),
  * It means you typed around 5 keys per second in attack mode,
  * And 6 keys per second in defence mode.
  * It will cause the rate of auto-handicap in battle.
  * 
  * VS COM
  * You can battle against Com.
  * if you set COM LV "NO DECREASE" in SETTING, the com lv won't decrease when you lost.
  * 
  * *** I really hope you enjoy this game!!!
  * 
  * Author: Kenichi UENO (Keno)
  * Sounds: Yoichi KANEKO / ザ・マッチメイカァズ2nd 【フリー効果音素材】
  * Test play and advices: LOGOSWARE PDG guys / kuroneko people / Nyafu
  * Japanese Words: Thierry Bézecourt
...
//		protected var enterUI:EnterUI = new EnterUI();
		...
		{
			Security.loadPolicyFile("http://174.37.179.162/crossdomain.xml");
...
//			words1.visible = false;
	...
	class OperationUI extends Sprite {
		protected var attackerBtn:Button = new Button(90, 25, "ATTACK");
		protected var defenderBtn:Button = new Button(90, 25, "DEFENCE");
...
//			reactor.self().setAttribute(Constants.PLAY_MODE, Score.playMode, Score.myGameRoom);
		...
		public function startInfo(isReconnect:Boolean = false ):void {
			...
			if ( typeNums[0] != -1 ) {
				infoUI.setInfoMessage("Typed: " + typeNums[0] + "(Accracy "+ int(typeNums[1]/typeNums[0]*10000)/100 +"%)");
			}
			infoUI.display();
			infoUI.updateSoloInfo("COM LEVEL: " + Score.cpuLV);
...
//						if ( e.getClient().isSelf() )gameRoom.setAttribute("status", Constants.STATUS_FINISHING);
					...
					if ( int(e.getChangedAttr().value) >= Constants.VICTORY_POINT ) {
						if ( Score.isObserve ) {
							infoUI.setInfoMessage("Red team won.");
							gameStage.end(true);
							removeListeners();
							return;
						}
						if ( ClientUtil.isTeamRed(reactor.self()) ) {
							infoUI.setInfoMessage("Congratulation! You won!");
							Score.win++;
							gameStage.end(true);
						} else {
							if (gameStage.cpu != null ) {
								gameStage.cpu.disable();
								infoUI.setInfoMessage("You lost.");
								if( !Score.comLock ) Score.cpuLV-= 0.5;
								if ( Score.cpuLV < 0.5 ) Score.cpuLV = 0.5;
							} else {
								infoUI.setInfoMessage("Your team lost.");
							}
...
//						if ( e.getClient().isSelf() )gameRoom.setAttribute("status", Constants.STATUS_FINISHING);
					...
					if ( int(e.getChangedAttr().value) >= Constants.VICTORY_POINT ) {
						if ( Score.isObserve ) {
							infoUI.setInfoMessage("Blue team won.");
							gameStage.end(true);
							removeListeners();
							return;
						}
						if ( ClientUtil.isTeamBlue(reactor.self()) ) {
							if (gameStage.cpu != null ) {
								gameStage.cpu.disable();
								infoUI.setInfoMessage("Congratulations! You won!");
								Score.cpuLV += 1;
							} else {
								infoUI.setInfoMessage("Congratulations! You won!");
							}
				...
				case "gameRoomStartTime": // ゲームルームの状態
						if ( Number(e.getChangedAttr().value) < ServerClockUtil.getTime() ) {
							if ( redNum == 0 && blueNum == 0 ) {
								infoUI.updateNormalRoomStatus("You can join.");
								infoUI.setGameRoomStatus(Constants.STATUS_CHATING);
							} else {
								infoUI.updateNormalRoomStatus("IN PROGRESS");
		...
		protected function onGameRoomCountDown(e:Event):void {
			...
			if( time > 0 ) {
				infoUI.updateNormalRoomStatus(Math.ceil(time / 1000) + "s to start");
		...
		protected function onGameUpdateClientAttribute(e:RoomEvent):void {
			if ( !gameRoom.clientIsInRoom(e.getClientID()) ) return;
			...
			if ( !Score.isSolo ) {
				...
				if ( blueHandicap != oldBlueHandicap || redHandicap != oldRedHandicap ) {
					if ( blueHandicap == redHandicap ) {
						gameStage.setInfoMsg( "No handicap." );
					} else {
						gameStage.setInfoMsg( "Handicap changed.\n" + (blueHandicap > 1?"BLUE":"RED") + " got " + int((Math.max(redHandicap, blueHandicap) - 1) * 100) + "% base bonus." );
					}
				}
...
*/
		...
		public function startNavigation(team:String):void {
			if ( team == Constants.TEAM_BLUE ) {
				if ( !Score.isObserve ) infoWindow.setMessage("We are blue team. Operation is beginning.");
			} else {
				if ( !Score.isObserve ) infoWindow.setMessage("We are red team. Operation is beginning.");
			}
		}
		// チームに入ってゲーム開始
...
//				operationUI.setMode(Constants.PLAY_MODE_ATTACKER); // デフォルトで攻撃モード
		...
		protected var _exitBtn:Button = new Button(140, 40, "BACK TO LOBBY");
	...
	class OptionWindow extends Sprite {
		...
		protected var alterEnterBtn:Button = new Button(100, 20, "ALT ENTER", true, true, 0x0);
		protected var switchToggleBtn:Button = new Button(100, 20, "ALT TAB", true, false, 0x0);
		protected var comLockBtn:Button = new Button(100, 20, "NO DECREASE", true, false, 0x0);
		...
		protected var EngBtn:Button = new Button(90, 20, "ENGLISH", true, false, 0x0);
		protected var BothBtn:Button = new Button(80, 20, "BOTH", true, false, 0x0);
		protected var JpnBtn:Button = new Button(80, 20, "JAPANESE", true, false, 0x0);
...
//			this.graphics.drawRoundRect(310, 170, 100, 113, 10);
			...
			attackTF.text = "ATTACK";
			defenceTF.text = "DEFENCE";
			titleTF.text = "BATTERY COLOR";
			baseColorTF.text = "COLOR 1";
			colorTF.text = "COLOR 2";
			handicapTitleTF.text = "HANDICAP SETTING";
			keyconfigTF.text = "SPACE KEY";
			comLockTF.text = "COM LEVEL";
			nameTF.text = "YOU NAME";
			languageTF.text = "ATTACK LANGUAGE";
...
//			veryHardBtn.addEventListener(MouseEvent.CLICK, onVeryHardClick);
	...
	class InfoUI extends Sprite {
		...
		protected var blueBtn:Button = new Button(60, 18, "BLUE", true, false, 0x0000FF);
		protected var redBtn:Button = new Button(60, 18, "RED", true, false, 0xFF0000);
		protected var autoJoinBtn:Button = new Button(100, 40, "", true, false, 0x999999);
		protected var optionBtn:Button = new Button(80, 20, "SETTING", true, false, 0x333333);
		...
		protected var soloBtn:Button = new Button(100, 20, "VS COM", true, false, 0x999999);
		...
		public function setGameRoomStatus(status:String):void {
			switch( status ) {
				case Constants.STATUS_CHATING:
						isObserve = false;
						autoJoinBtn.setText( Score.isWaitingAtLobby?"WAITING":"AUTO JOIN" );
						autoJoinBtn.setBorderColor( 0xFFFFFF, -1 );
					break;
				case Constants.STATUS_PLAYING:
						isObserve = true;
						autoJoinBtn.setText( "OBSERVE" );
...
//			optionWindow.setHandicap();
		...
		public function noticeClosed():void {
			chatWindow.setInfoMessage("CONNECTION LOST.");
		}
		public function display():void {
			...
			if ( Score.lastLevel < Score.getLevel() ) {
				chatWindow.setInfoMessage("LV UP! You got another color!");
			}
			Score.lastLevel = Score.getLevel();
		}
...
//			redBtn.selected = true;
		...
		public function setLobbyUserNum(num:int ):void {
			lobbyUserNumText.text = "" + num + ((num>1)?" users":" user") + " is connecting!";
		}
...
//			users.defaultTextFormat = input.defaultTextFormat = coming.defaultTextFormat = new TextFormat(null, null, 0xFFFFFF);
			...
			if ( isReconnect ) {
				defaultText += getNowTimeFormat() + " *** Reconnected.\n";
			} else {
				defaultText += getNowTimeFormat() + " *** Welcome to TypeShoot! ***\n";
				defaultText += getNowTimeFormat() + " * ver 1.4.5\n";
				defaultText += getNowTimeFormat() + " * type /h to see hot key list.\n";
			}
		...
		protected function onKeyDown(e:KeyboardEvent):void {
			if ( (e.keyCode == Keyboard.ENTER) && ( input.text != "" ) ) {
				switch( input.text ) {
					...
					case "/help":
					case "/h":
						setInfoMessage("[IN LOBBY]");
						setInfoMessage("/h: display this list");
						setInfoMessage("/v: display the version");
						setInfoMessage("[IN GAME]");
						setInfoMessage("F2: begin team chat");
						setInfoMessage("Tab: toggle attack/defence mode");
						if( Score.ALTER_ENTER == Keyboard.SPACE ) {
							setInfoMessage("Space: shoot a word");
						} else {
							setInfoMessage("Space: toggle attack/defence mode");
						}
						setInfoMessage("【IN GAME, DEFENCE MODE】");
						setInfoMessage("F12: toggle defence range");
						setInfoMessage("ESC: reset inputting");
						break;
					default:
...
//			return "ＭＳ ゴシック";
	...
	class Score {
		...
		public static var japaneseRate:Number = 0; // SO保存
...
//			timeDiff = serverTime - Number(getTimer());
	...
	class ImageManager {
		...
		public static function init():void {
			...
			SasakoA.load(new URLRequest("http://174.37.179.162/TypeShoot/sasako_1.png"));
			...
			SasakoB.load(new URLRequest("http://174.37.179.162/TypeShoot/sasako_2.png"));
			...
			MikiA.load(new URLRequest("http://174.37.179.162/TypeShoot/miki_1.png"));
			...
			MikiB.load(new URLRequest("http://174.37.179.162/TypeShoot/miki_2.png"));
		}
