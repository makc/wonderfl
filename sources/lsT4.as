...
package {
    ...
    public class FlashTest extends Sprite {
        ...
        private static const BGCOLOR:uint = 0xFF000033;
        private function init( e:Event ):void {
            removeEventListener( Event.ADDED_TO_STAGE, init );
            
            stage.quality = StageQuality.LOW;
            
            _canvas = new BitmapData( stage.stageWidth, stage.stageHeight, true, BGCOLOR );
        ...
        private function preRendering():void {
            var origin:Sprite = new BallView( 5, 0xFFff0000 );
            var max:Number = 60;
            for( var i:int=1; i<max; ++i ) {
                var bmpd:BitmapData = new BitmapData( max, max, true, 0x00 );
                bmpd.draw( origin, new Matrix( i*0.1, 0, 0, i*0.1, max*0.5, max*0.5 )  );
                PRERENDER.push( bmpd );
                
                //_canvas.draw( bmpd, new Matrix( 1, 0, 0, 1, i*max, 0 ) );
            }
        }    
        private function createElements():void {
            ...
            for( var i:int = 0; i< wlength; ++i ) {
                for( var j:int = 0; j< hlength; ++j ) {
                    var ball:Ball = new Ball( ( i - wlength*0.5 ) * intervalX, 0, ( j - hlength*0.5 ) * intervalY, 5, 0xFF33FF99 );
                    addCastChild( ball );
                }
            }
        ...
        private function calcurate():void {
            _camera.rotateX +=  (( 90* (mouseY - CENTER.y) / 435 +20 ) - _camera.rotateX  )* 0.05 ;
            _camera.rotateY +=  (( -120 * (mouseX - CENTER.x) / 435 ) - _camera.rotateY ) * 0.03;
            _camera.y +=  (( -200 * (mouseY - CENTER.y) / 435 -400 ) - _camera.y ) * 0.03;
        ...
        private static const ALPHA:ColorTransform= new ColorTransform( 0.96, 0.90, 0.92, 1, 0, 0, 1, 0 );
        private function update(e:Event):void {
            ...
            _canvas.lock();
            _canvas.applyFilter( _canvas, _canvas.rect, O, BLUR );
            _canvas.colorTransform( _canvas.rect, ALPHA );
            for each( var cast:Cast in _renderList ) {
                ...
                var view:BitmapData= PRERENDER[ Math.min( Particle( cast ).radius, PRERENDER.length-1 )];
                
                var color :ColorTransform = getColor( ( 255*scale ) << 24 | Particle( cast ).color );
                _canvas.draw( view, new Matrix( scale, 0, 0, scale, screenX, screenY ), color );
            }
            _canvas.unlock();
        }
        private function getColor( color:uint, strong:Number = 1.0 ):ColorTransform {
            var k:Number = 1.0 - strong;
            var alpha:Number = color >> 24 & 0xFF *strong;
            var red:Number = color >> 16 & 0xFF *strong;
            var green:Number = color >> 8 & 0xFF *strong;
            var blue:Number = color & 0xFF *strong; 
            return new ColorTransform( k, k, k, k, red, green, blue, alpha *0.8 );
        }
    }
}
